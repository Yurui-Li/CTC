...
class DOLCLearner:
	def __init__(self, mac, scheme, logger, args):
		...
		print("The model has ",sum(p.numel() for p in self.params)," parameters.")

		print('Mixer Size: ')
		print("The Mixer has ",sum(p.numel() for p in self.mixer.parameters())," parameters.")
	...
	def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, per_weight=None):
		...
		gates = th.stack(gates, dim=1) # [b,s,a,n_expert]
		...
		# load_balance
		mscores = gates / gates.sum(dim=-1,keepdim=True) # [b,s,a,n_expert]
		mprob = mscores.mean(dim=1) # [b,a,n_expert]
		mindices = th.topk(gates, self.args.topk, dim=-1)[1] # [b,s,a,topk]
		mmask2 = th.zeros_like(gates) # [b,s,a,n_expert]
		mmask2.scatter_(dim=-1, index=mindices, value=1) # [b,s,a,n_expert]
		mf_i = mmask2.mean(dim=1)  # [b,a,n_expert]
		mbal = (mf_i * mprob).sum(dim=-1).mean()
		...
		if (episode_num - self.last_target_update_episode) / self.args.target_update_interval >= 2.0:
			th.cuda.empty_cache()
